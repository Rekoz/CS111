...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if (current->waiter != 0) // if there is a process waiting
		  {
		    current->waiter->p_state = P_RUNNABLE; // unblock it
		    current->waiter->p_registers.reg_eax = current->p_exit_status;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	/* each process's stack size is fixed and the pid indicates the nth process. 
	   By multiplying the stack size by the pid, we can get the stack top easily. */
	...
	/* As indicated by the diagram above, (src_stack_bottom - src_stack_top) calculates
	   the offset from the stack top. By adding this offset to dest_stack_top, we get
	   the dest_stack_bottom. */
 ...
 * do_newthread
 *
 *   This function creates a new thread with empty stack, and starts by
 *   executing the start_function function.
 *
 *****************************************************************************/

static pid_t
do_newthread(process_t *parent, uint32_t start_function)
{
